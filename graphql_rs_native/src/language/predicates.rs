...
pub fn is_type_node(node: &'_ AST<'_>) -> bool {
    match node {
        AST::NamedType(_) | AST::NonNullType(_) | AST::ListType(_) => true,
        _ => false,
    }
}
pub fn is_type_system_definition_node(node: &'_ AST<'_>) -> bool {
    match node {
        AST::DirectiveDefinition(_) | AST::SchemaDefinition(_) => true,
        _ => is_type_definition_node(node),
    }
}
pub fn is_type_definition_node(node: &'_ AST<'_>) -> bool {
    match node {
...
pub fn is_type_system_extension_node(node: &'_ AST<'_>) -> bool {
    match node {
        AST::SchemaExtension(_) => true,
        _ => is_type_extension_node(node),
    }
}
pub fn is_type_extension_node(node: &'_ AST<'_>) -> bool {
    match node {
